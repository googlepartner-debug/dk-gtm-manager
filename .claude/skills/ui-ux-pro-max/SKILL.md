...
## DK GTM Manager — Contraintes charte (PRIORITÉ ABSOLUE)

Ces règles surchargent toute recommandation générique du skill. Ne jamais les contourner.

- **Couleurs** : utiliser exclusivement les tokens HSL définis dans `src/index.css` (`@theme`). Ne pas introduire de nouvelles couleurs. Palette : foreground `hsl(220 13% 9%)`, muted-fg `hsl(220 13% 46%)`, border `hsl(220 13% 91%)`, brand `hsl(213 94% 50%)`, success `hsl(142 60% 40%)`, warning `hsl(27 96% 55%)`, error `hsl(0 84% 60%)`.
- **Tailwind v4** : config dans `src/index.css` via `@theme`. Pas de `tailwind.config.ts`. Pas de classes arbitraires non-HSL.
- **Icônes** : Lucide React uniquement (déjà installé). Pas d'emoji, pas d'autres icon packs.
- **Style global** : dashboard interne entreprise — minimaliste professionnel, pas de glassmorphism/claymorphism/brutalisme. Border-radius cohérent avec le reste (`rounded-lg` = 8px).
- **Animations** : micro-interactions légères (150–200ms), pas d'animations décoratives.
- **Stack** : React 19 + Vite + TypeScript + Tailwind v4 + Zustand 5. Appliquer les recommandations du skill via `--stack react` et `--stack html-tailwind`.
