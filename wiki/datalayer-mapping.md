...
- **Focus Mode** : isole un funnel, ligne de flux qui casse visuellement si la complétion d'une étape passe sous 95%. **Éditable depuis le 2026-07-13** — plus une constante en dur : bouton ⚙ à côté du toggle ouvre `FocusModeEditor` (réordonner/supprimer/ajouter des events), persisté par client dans `datalayerStore.focusEvents`. Défaut si rien de custom : `DEFAULT_FUNNEL_STEPS` (`view_item_list → view_item → begin_checkout → purchase`) dans `constants/ga4Events.ts`.
