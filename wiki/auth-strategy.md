...
## Restriction d'accès (phase interne, depuis 2026-07-10)

Deux couches, pas une seule :

1. **Réelle barrière** : écran de consentement OAuth du projet GCP `gtm-wbncv54-ngq1n` passé en type **Interne** — Google refuse toute connexion hors de l'organisation `@digitalkeys.fr` avant même l'exécution du code JS, donc infalsifiable côté client. Réglage fait dans la console GCP (Ron), rien côté code.
2. **Garde-fou de confort seulement** : `ALLOWED_EMAILS`/`isEmailAllowed()` dans `src/lib/auth.ts`, restreint à `googlepartner@digitalkeys.fr` — vérifié au login (`auth-store.ts`, révoque le token Google si refusé) et au chargement d'une session persistée (`loadAuthState()`). Contournable en éditant le bundle JS ou le localStorage — sert juste à éviter qu'un autre consultant DK légitime se connecte par erreur à cet outil précis pendant la phase de validation interne (§12 du PRD).

**À faire avant l'onboarding PFS** (§10/§12 du PRD) : un projet GCP en "Interne" ne peut accepter que des comptes de l'organisation — repasser en "Externe" (ou projet séparé) sera nécessaire pour que les comptes `@perfectstay.com` puissent se connecter. `ALLOWED_EMAILS` à étendre à ce moment-là.

Voir [[deferred-features]] pour ce qui reste à construire (workspace last-modified, usage tracking) et [[profiles]] pour le détail du système de profils.
