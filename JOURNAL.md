...
Actions ajoutées : `gtmStore.seedTestContainer()` (ajoute sans écraser le monitoringData existant, contrairement à `loadMockMonitoringData` qui remplace tout), `datalayerStore.loadTestContainer()`, `trackingPlanStore.loadPlan(clientId, plan)` (généralisation de `loadMockPlan`, qui l'appelle maintenant en interne). Testé via Playwright de bout en bout : clic sur le bouton → DataLayer Mapping bascule sur "Container de test (démo)" avec 3 events, Plan de tracking affiche Vérifié/Implémenté/Planifié sur les 6 events comme prévu, Kanban fonctionne, un second clic ne duplique rien (idempotent).
