...
**Endpoint Google Sheets déployé par Ron mais jamais fonctionnel — abandonné, retour à Supabase plus tard.** Ron a créé le Sheet et déployé `dl-mapping-sheets-endpoint.gs.js` en Apps Script. Premier test `curl` : `ReferenceError: src is not defined` — le fichier collé ne contenait que le chemin du fichier (`src/features/...gs.js`), pas son contenu ; corrigé en fournissant le code brut à coller. Après redéploiement, nouvelle erreur systématique sur 3 tentatives (`curl` direct, avec `--post302/--post303`, en suivant la redirection `script.googleusercontent.com` à la main) : page Drive "Impossible d'ouvrir le fichier" (HTTP 405) — signature typique d'un déploiement Apps Script réglé sur "Toute personne disposant d'un compte Google" plutôt que "Tout le monde" (le tag s'exécute dans le navigateur d'un visiteur anonyme, sans compte Google, donc bloqué par ce réglage). Pas résolu — **Ron a choisi d'abandonner cette piste plutôt que de continuer à déboguer**, en attendant que la vraie Phase B Supabase soit montée. Le template package et le collecteur restent en place et fonctionnels (mode 100% local, sans l'endpoint) ; la variable `DL Mapping - Sheets Endpoint` reste vide par défaut, donc rien à changer côté outil si l'idée est reprise plus tard — il suffira de vérifier le réglage d'accès avant de re-tester.

Décision : reprendre le travail avec les **données simulées** (mock Noviscore déjà en place) en attendant Supabase, plutôt que de bloquer sur la capture réelle.

---

## 2026-07-14 — Plan de démo pour le Plan de tracking

Nouveau `src/data/tracking-plan-mock.ts` (`MOCK_TRACKING_PLAN`, clientId `noviscore`) : 6 events choisis pour recouper naturellement avec les mocks existants et obtenir un vrai mix de statuts sans rien inventer côté cross-référencement — `purchase`/`view_item` → Vérifié (bonne complétion dans `datalayer-mock.ts`), `add_shipping_info`/`add_payment_info`/`view_cart`/`sign_up` → Planifié (0 occurrence ou jamais capté). Priorités et owners volontairement variés (critique/important/normal/optionnel, Paid Media/SEO/Merch/CRM) pour avoir de quoi regarder à l'œil en itérant sur l'UI.

Note : "Implémenté" ne peut pas sortir de ce seed seul — le mock Monitoring existant (`monitoring-mock.ts`) couvre des containers compagnies aériennes, pas les containerId Noviscore (`GTM-NOVIFR01` etc.), donc aucun recoupement possible tel quel. Documenté dans le fichier ; pas corrigé pour l'instant (pas nécessaire pour itérer sur l'UI avec 2 statuts sur 3).

Nouvelle action `trackingPlanStore.loadMockPlan(clientId)` + bouton "Charger un plan de démo" (visible seulement si `clientId === 'noviscore'`, à côté de "Nouveau plan"). Testé : les 6 events se chargent avec les statuts attendus.

---

## 2026-07-14 (suite) — Fusion DataLayer Mapping / Plan Kanban, drag&drop Focus Mode, container de test cross-store

Trois chantiers demandés ensemble par Ron.

**Fusion DataLayer Mapping + Plan Kanban en une page** — les deux vivaient sur deux routes séparées (`/dashboard/datalayer-mapping` et `/dashboard/datalayer-kanban`) avec chacune son propre sélecteur client/site, alors qu'elles tournent sur le même `datalayerStore`. `DatalayerKanbanPage.tsx` exporte maintenant `KanbanView()` (sans header ni sélecteur propres) et se rend en dessous d'un toggle "Vue Liste / Vue Kanban" dans `DataLayerMappingPage.tsx`, qui possède désormais le seul sélecteur client/site partagé par les deux vues. Route `datalayer-kanban` conservée en redirection (`<Navigate>`) vers `datalayer-mapping` pour ne pas casser un lien existant ; entrée "Plan Kanban" retirée de la sidebar. Testé via Playwright : sélecteur partagé, bascule de vue, Kanban fonctionnel sous la nouvelle page.

**Drag & drop pour réordonner les étapes du Focus Mode** — les boutons haut/bas de `FocusModeEditor` (Kanban, réglage du funnel PRD §14.6) remplacés par du drag & drop HTML5 natif (`draggable`, `onDragStart/onDragOver/onDrop/onDragEnd`, pas de librairie pour une poignée d'étapes). Testé via Playwright (`browser_drag`, pas de `dispatchEvent` manuel qui ne déclenche pas les handlers React de façon fiable) : réordonner `view_item_list` en dernière position déplace bien l'étape, et le connecteur de funnel (`FunnelConnector`) reflète immédiatement le nouvel ordre.

**Container de test / bac à sable** — nouveau `src/data/test-container-mock.ts` : jeu de données 100% fictif, sélectionnable en un clic, qui peuple GTM Monitoring + DataLayer Mapping + Plan de tracking d'un coup via un point d'entrée unique (`src/lib/testContainerSeed.ts` → `seedTestContainer()`, bouton "Charger un container de test" sur DataLayer Mapping et Plan de tracking). Contrairement aux mocks existants (Noviscore, compagnies aériennes) qui ne se recoupent jamais entre eux, celui-ci utilise volontairement le même identifiant comme `containerId` GTM et comme `siteId` DataLayer Mapping (`GTM-TESTDEMO01`) — ça comble le trou documenté dans `tracking-plan-mock.ts` où le statut "Implémenté" ne pouvait jamais apparaître faute de recoupement. Le seed produit les 3 statuts à la fois, avec deux causes réelles distinctes pour "Implémenté" (`add_to_cart` : jamais recoupé côté DataLayer Mapping ; `begin_checkout` : recoupé mais occurrences=0/statut 'problem', donc "tag posé mais rien ne remonte en prod").

Actions ajoutées : `gtmStore.seedTestContainer()` (ajoute sans écraser le monitoringData existant, contrairement à `loadMockMonitoringData` qui remplace tout), `datalayerStore.loadTestContainer()`, `trackingPlanStore.loadPlan(clientId, plan)` (généralisation de `loadMockPlan`, qui l'appelle maintenant en interne). Testé via Playwright de bout en bout : clic sur le bouton → DataLayer Mapping bascule sur "Container de test (démo)" avec 3 events, Plan de tracking affiche les 3 statuts sur les 6 events comme prévu, Kanban fonctionne, un second clic ne duplique rien (idempotent).

**Renommage des statuts du Plan de tracking — "Vérifié" ne voulait pas dire ce que ça devrait dire.** Ron a pointé un décalage sémantique en testant le container de test : pour lui, un `dataLayer.push()` réellement détecté en prod (ce que le code appelait "Vérifié") correspond à "le dev a implémenté le push" — pas à une vérification. Réflexion sur un modèle à 4 niveaux avec validation manuelle par l'analyst (nouveau champ, pas juste du recoupement auto), mais Ron a tranché pour l'option la plus simple : garder le recoupement 100% automatique, juste renommer. Nouveau modèle `EventStatus` (`trackingPlan.types.ts`) : `planifie` → `configure` (tag GTM posé, ex-"implemente") → `implemente` (dataLayer réel détecté, ex-"verifie"). Mis à jour partout : `TrackingPlanPage.tsx` (labels, badges, logique `useEventStatus`, tooltip, panneau de détail), `PRD_TrackingPlan.md` §5, commentaires de `tracking-plan-mock.ts` et `test-container-mock.ts`. Revérifié via Playwright sur le container de test : purchase/view_item → Implémenté, add_to_cart/begin_checkout → Configuré, view_item_list/sign_up → Planifié.

---

## 2026-07-14 (suite 4) — Lancement du chantier UX/UI page par page, Containers fait

Ron veut un vrai passage UX/UI dédié par page plutôt qu'un vernis partout d'un coup — ordre retenu : Containers → Packages → Déployer → Monitoring → DataLayer Mapping → Plan de tracking (voir `wiki/ux-audit-chantier.md`, nouvelle page qui trackera ce chantier au fil des sessions).

**Audit Containers** : capture réelle de l'état "session expirée" (le faux token Playwright renvoie un vrai 401 sur l'API GTM), plus lecture de code pour les autres états. 4 points relevés, Ron a choisi de tous les traiter :

1. **Recherche/filtre containers** (`ContainersPage.tsx`) — champ de recherche par nom/ID public au-dessus de la liste, affiché seulement au-delà de 5 containers pour ne pas ajouter de bruit sur un petit compte. Compteur "X résultat(s) sur Y", état vide dédié.
2. **Header — séparateur entre profil DK et compte Google** (`Header.tsx`) — la pill de profil (espace de travail, plusieurs consultants DK partagent le même compte Google) et le bloc identité Google se ressemblaient trop l'un à côté de l'autre. Ajout d'un simple séparateur vertical, pas de refonte.
3. **État "session expirée" repensé** — remplaçait un petit encart rouge perdu en haut d'une page à 90% vide. Nouveau panneau centré pleine largeur (icône, titre, explication, CTA), qui affiche en plus le nom du dernier compte consulté quand disponible. Nécessitait de persister ce nom : `gtmStore.recentAccountName`, nouveau champ dans `dk_gtm_recent` aux côtés de `accountId`/`containerIds` (mis à jour à `selectAccount` et propagé sur les 3 autres call sites de `saveRecent` pour ne pas le perdre en cours de session).
4. **Fallback statique (`gtm-static.ts`) clarifié, pas corrigé** — vérification qu'il est en fait inatteignable via l'UI : `RequireAuth` bloque tout `/dashboard/*` tant qu'`accessToken` est vide, donc `fetchAccounts` reçoit toujours un vrai token dès qu'on voit la page. Pas touché au guard (blast radius bien plus large que demandé) — documenté dans `CLAUDE.md` comme point en attente d'une décision produit plutôt que laissé comme une affirmation fausse.

**Testé via Playwright** : état session-expirée avec/sans `recentAccountName` (le nouveau panneau bascule correctement entre les deux textes), recherche filtrant 12 containers de test injectés directement dans le store (`useGTMStore.setState`, exposition temporaire sur `window` retirée après vérification) — filtre "Demo 7" → 1 résultat, filtre sans match → état vide dédié.

**CTA "Publier" déplacé dans le header, en cours de session.** Ron a relevé deux points sur ce qu'on venait de livrer : le mot "déploiement" dans "Aller au déploiement →" n'est pas approprié à l'étape de sélection de containers (rien n'est publié à ce stade, on navigue juste vers l'étape suivante), et le badge numérique à côté de "Déployer" dans la sidebar n'a pas sa place — il veut un vrai CTA "Publier" en haut à droite du header global, comme le bouton Submit de GTM. Implémenté : retiré le badge `isDeploy && selectedCount > 0` de `Sidebar.tsx` (et le `useGTMStore` devenu inutile), retiré les deux boutons "Aller au déploiement →" de `ContainersPage.tsx` (header et bloc dupliqué en bas — gardé uniquement le texte informatif "X containers sélectionnés", `navigate`/`useNavigate` devenus inutiles), ajouté un bouton "Publier [N]" dans `Header.tsx` (visible dès qu'une sélection existe, sur n'importe quelle page, navigue vers `/dashboard/deploy`). Testé via Playwright avec des containers injectés directement dans le store : le CTA apparaît bien dans le header avec le bon compteur, clic → navigation effective vers `/dashboard/deploy`.

**Entrée "Déployer" retirée de la sidebar.** Dernier ajustement du même fil : devenue redondante avec le CTA "Publier" du header, retirée du tableau `nav` de `Sidebar.tsx`. La route `/dashboard/deploy` reste intacte, seul point d'entrée restant : le header.

---

## 2026-07-14 (suite 5) — Chantier UX/UI, deuxième page : Packages

Deuxième page de l'ordre retenu (voir `wiki/ux-audit-chantier.md`). Audit avec captures réelles (liste vide, modal "Depuis un template", éditeur peuplé via le vrai template GA4 Ecommerce Standard) — trois bugs réels trouvés, corrigés directement sans arbitrage (pas des choix de design) :

1. **Badge client vide/cassé** — `client: ''` sur le template GA4 Ecommerce Standard (`package-templates.ts`) affichait un badge violet vide dans la liste. `PackagesPage.tsx` cache maintenant le badge quand `pkg.client` est vide.
2. **Chemin de fichier dans une description utilisateur** — la carte du template "DataLayer Mapping — Collecteur" affichait littéralement `src/features/datalayer-mapping/gtm-tag/dl-mapping-sheets-endpoint.gs.js` dans le texte du picker. Raccourci, renvoie vers le PRD à la place.
...
## 2026-07-14 (suite 8) — Chantier UX/UI renforcé avec le skill ui-ux-pro-max

Ron a demandé de reprendre le chantier avec le skill `ui-ux-pro-max` (checklist de 99 règles UX priorisées : accessibilité, touch/interaction, layout, typographie/couleur, animation, formulaires, navigation, charts) et de repasser les pages déjà faites avec cette grille avant de continuer. Le skill était déjà installé dans `.claude/skills/ui-ux-pro-max/` (CLI Python, `python .claude/skills/ui-ux-pro-max/scripts/search.py`) — pas eu besoin de le cloner. Important : ses recommandations génériques de palette/typo (`--design-system`) ne sont PAS appliquées telles quelles — la charte DK reste la référence ; seule la partie UX/accessibilité du skill sert de grille.

**Passe accessibilité transversale** sur les 4 pages déjà auditées + leurs composants directs : règle "ARIA Labels" (sévérité High) — tout bouton dont le seul contenu est une icône SVG doit avoir un `aria-label`. Un agent Explore a scanné `ContainersPage.tsx`, `PackagesPage.tsx`, `DeployPage.tsx`, `MonitoringPage.tsx`, `Header.tsx`, `Sidebar.tsx`, tous les composants de `components/monitoring/`, `BulkRenameModal.tsx`, `EntityDrawer.tsx`, `DiffView.tsx`, `GA4CoverageMatrix.tsx`. 9 boutons trouvés sans `aria-label` (1 faux positif écarté : le bouton "Proposer une amélioration" de la sidebar a bien un texte visible). Un 9ème trouvé manuellement en plus (bouton retour de `PackageEditor` dans `PackagesPage.tsx`, raté par l'agent). Corrigés : boutons fermer de modale/drawer (`PackagesPage.tsx` template picker, `BulkRenameModal.tsx`, `EntityDrawer.tsx`, `DistributionTab.tsx` ×2), boutons retour (`DeployPage.tsx`, `PackagesPage.tsx`), retirer un renommage (`ContainersPage.tsx`), effacer un filtre (`DiffView.tsx`) — tous avec un libellé français cohérent. Vérifié via Playwright que le bouton de fermeture de la modale template expose bien le nom accessible "Fermer".

Autres points vérifiés avec la checklist du skill, sans problème trouvé : contraste `--muted-fg` sur fond blanc (≈4.7:1, conforme AA de justesse), focus states (déjà gérés globalement par `Button.tsx` via `focus-visible:ring-2`), badges de statut jamais couleur seule (toujours accompagnés d'un texte).

---

## 2026-07-14 (suite 9) — Chantier UX/UI, cinquième et sixième pages : DataLayer Mapping et Plan de tracking

Dernières pages du chantier, auditées avec la méthode renforcée (skill + agent Explore dédié) dès le départ. Un second agent a scanné `DataLayerMappingPage.tsx`, `DatalayerKanbanPage.tsx`, tous les composants de `features/datalayer-mapping/components/`, et `TrackingPlanPage.tsx` — 4 boutons icône-seule sans `aria-label` trouvés et corrigés (`DatalayerKanbanPage.tsx` retirer une étape du Focus Mode, `VariableDrillDown.tsx` retour, `EventDetailDrawer.tsx` fermer le tiroir, `TrackingPlanPage.tsx` supprimer un screenshot).

**Cul-de-sac réel trouvé et corrigé sur Plan de tracking.** `TrackingPlanPage.tsx` affichait juste "Sélectionne un client." sans aucune action possible quand `activeClientId` est vide — ce qui arrive systématiquement sur un profil neuf qui n'a jamais visité DataLayer Mapping avant, puisque rien n'auto-charge de client depuis cette page (contrairement à `DataLayerMappingPage.tsx` qui a son propre effet `loadMockData()` au montage, et dont le sélecteur reste visible même dans ce cas). Remplacé par un vrai état vide avec deux actions : "Aller choisir un client →" (vers DataLayer Mapping) et, si aucun client n'existe nulle part, "Charger un container de test" directement inline. Testé via Playwright avec un profil 100% vierge créé pour l'occasion : la page était bloquée avant, se débloque maintenant en un clic.
