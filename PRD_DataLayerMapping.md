...
**Mode local (2026-07-13, en attendant la Phase B) — pas d'endpoint réseau requis pour commencer un vrai audit.** Le buffer s'accumule dans le `localStorage` du site audité. Deux fonctions exposées sur `window` pendant une session d'audit : `__dlMappingExport()` télécharge un JSON, `__dlMappingClear()` vide le buffer. Le JSON s'importe ensuite dans l'outil via le bouton "Importer un export" (page DataLayer Mapping) — `src/features/datalayer-mapping/utils/importCollectorExport.ts` recalcule events/variables/anomalies à partir des occurrences brutes, en préservant priorité/statut/notes déjà saisis à la main sur un événement déjà connu.

**Angle mort identifié (2026-07-13)** : le mode local ne fonctionne que pour une session d'audit manuelle (Ron navigue lui-même, en Preview GTM) — publier ce tag en production instrumenterait chaque vrai visiteur (écriture dans son navigateur, en permanence) sans que personne ne puisse jamais relire cette donnée, puisqu'elle reste dans le navigateur de chacun. Ne jamais publier ce tag tant qu'aucune destination centralisée n'est configurée.

**Destination intermédiaire retenue : Google Sheets, pas Supabase** — Ron veut pouvoir instrumenter du vrai trafic sans attendre la Phase B. `src/features/datalayer-mapping/gtm-tag/dl-mapping-sheets-endpoint.gs.js` (script Apps Script, à déployer manuellement — pas automatisable depuis l'outil) reçoit les batches et ajoute une ligne par occurrence dans un Google Sheet. Le tag POST vers `{{DL Mapping - Sheets Endpoint}}` en `Content-Type: text/plain` (JSON déclencherait un preflight CORS qu'Apps Script ne gère pas) si la variable est renseignée, sinon reste en mode 100% local (localStorage uniquement — comportement par défaut du package). Limites assumées : quotas d'exécution Apps Script/jour (déjà atténué par le batching existant, 5s/20 events), performance dégradée au-delà de quelques centaines de milliers de lignes dans un Sheet, URL `/exec` non authentifiée par construction (payload déjà anonymisé côté tag, donc pas de PII même en cas d'abus). Quand la Phase B Supabase existera, remplacer la valeur de cette variable par l'URL de l'Edge Function — le format de payload ne change pas.
